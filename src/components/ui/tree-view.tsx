...
const TreeView = React.forwardRef<HTMLDivElement, TreeProps>(
  (
    ...
    return (
      <div className={cn("relative h-full min-h-0 p-2", className)}>
        <TreeItem
          data={data}
          ref={ref}
          ...
          {...props}
        />
        <div className="w-full h-[48px]" onDrop={() => handleDrop({ id: "", name: "parent" })} />
      </div>
    )
  }
)
  ...
  return (
    <AccordionPrimitive.Root type="multiple" value={value} onValueChange={(s) => setValue(s)}>
      <AccordionPrimitive.Item value={item.id}>
        ...
        <AccordionContent className="ml-4 pl-2 border-l border-dashed border-border/60">
          <TreeItem
